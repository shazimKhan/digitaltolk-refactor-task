    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if ($data['immediate'] == 'no') {
                ...
                if (!isset($data['customerPhoneType']) && !isset($data['customerPhysicalType'])) {
                    $response['status'] = 'fail';
                    $response['message'] = "Du måste göra ett val här";
                    $response['field_name'] = "customerPhoneType";
                    return $response;
                }
            ...
            if (isset($data['customerPhoneType'])) {
                $data['customerPhoneType'] = 'yes';
            } else {
                $data['customerPhoneType'] = 'no';
            }

            if (isset($data['customerPhysicalType'])) {
                $data['customerPhysicalType'] = 'yes';
                $response['customerPhysicalType'] = 'yes';
            } else {
                $data['customerPhysicalType'] = 'no';
                $response['customerPhysicalType'] = 'no';
            }
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobToData($job)
    {
        ...
        $data['customerPhoneType'] = $job->customerPhoneType;
        $data['customerPhysicalType'] = $job->customerPhysicalType;
    ...
    public function getPotentialJobIdsWithUserId($userId)
    {
        ...
        foreach ($jobIds as $k => $v)     // checking translator town
        {
            ...
            if (($job->customerPhoneType == 'no' || $job->customerPhoneType == '') && $job->customerPhysicalType == 'yes' && $checktown == false) {
                unset($jobIds[$k]);
            }
        }
    ...
    public function sendNotificationTranslator($job, $data = [], $excludeUserId)
    {
        ...
        $logger->addInfo('Push send for job ' . $job->id, [$translatorArray, $delpayTranslatorArray, $msg_text, $data]);
        $this->sendPushNotificationToSpecificUsers($translatorArray, $job->id, $data, $msg_text, false);       // send new booking push to suitable translators(not delay)
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        if ($job->customerPhysicalType == 'yes' && $job->customerPhoneType == 'no') {
            // It's a physical job
            $message = $physicalJobMessageTemplate;
        } else if ($job->customerPhysicalType == 'no' && $job->customerPhoneType == 'yes') {
            // It's a phone job
            $message = $phoneJobMessageTemplate;
        } else if ($job->customerPhysicalType == 'yes' && $job->customerPhoneType == 'yes') {
     ...
     * @param $is_need_delay
     */
    public function sendPushNotificationToSpecificUsers($users, $jobId, $data, $msg_text, $is_need_delay)
    {
...
//        }
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($job->customerPhysicalType == 'yes')
            $msg_text = array(
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if (!is_null($currentTranslator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($currentTranslator) && ((isset($data['translator']) && $currentTranslator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $logData[] = [
                    'oldTranslator' => $currentTranslator->user->email,
            ...
            if ($translatorChanged)
                return ['translatorChanged' => $translatorChanged, 'newTranslator' => $newTranslator, 'log_data' => $logData];
     ...
     * @param $duration
     */
    // unused Private Function becuase we can only used private function within the class
...
//                Event::fire(new JobWasCanceled($job));
    ...
    public function getPotentialJobs($cUser)
    {
        ...
        $jobIds = Job::getJobs($cUser->id, $jobType, 'pending', $userLanguage, $gender, $translator_level);
...
//        $jobs = TeHelper::convertJobIdsInObjs($jobIds);
    ...
    public function endJob($postData)
    {
        $completedDate = date('Y-m-d H:i:s');
        $jobId = $postData["job_id"];
        $completedDate = Job::with('translatorJobRel')->find($jobId);

        if($completedDate->status != 'started')
            return ['status' => 'success'];

        $dueDate = $completedDate->due;
        $start = date_create($dueDate);
        $end = date_create($completedDate);
        $diff = date_diff($end, $start);
        $interval = $diff->h . ':' . $diff->i . ':' . $diff->s;
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        $cUser = $requestData['__authenticatedUser'];
        ...
        if ($cUser && $cUser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (isset($requestData['physical'])) {
                $allJobs->where('customerPhysicalType', $requestData['physical']);
                $allJobs->where('ignore_physical', 0);
            }

            if (isset($requestData['phone'])) {
                $allJobs->where('customerPhoneType', $requestData['phone']);
                if(isset($requestData['physical']))
            ...
            if (isset($requestData['booking_type'])) {
                if ($requestData['booking_type'] == 'physical')
                    $allJobs->where('customerPhysicalType', 'yes');
                if ($requestData['booking_type'] == 'phone')
                    $allJobs->where('customerPhoneType', 'yes');
            }
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        $cUser = Auth::user();
        // $consumer_type = TeHelper::getUsermeta($cUser->id, 'consumer_type'); this is unsed line 
        ...
        return ['allJobs' => $allJobs, 'languages' => $languages, 'allCustomers' => $allCustomers, 'alltTranslators' => $allTranslators, 'requestData' => $requestData];
    }
